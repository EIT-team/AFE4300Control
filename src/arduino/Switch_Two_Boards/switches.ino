...
void programswitches2(int sourcechnA, int sinkchnA, int sourcechnB, int sinkchnB, int maxpins)
{
	...
	for (int j = maxpins; j > 0; j--) {
		...
		if (sourcechnA == j)  digitalWrite(DINp, HIGH);
		if (sinkchnA == j)  digitalWrite(DINn, HIGH);
		if (sourcechnB == j)  digitalWrite(DINp, HIGH);
		if (sinkchnB == j)  digitalWrite(DINn, HIGH);
...
//	digitalWrite(SYNC, LOW);
//	digitalWrite(SYNC, HIGH);
//	programswitches(0, 0, 0, 0, TotalPins); //program dem switches
